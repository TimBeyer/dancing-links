/**
 * New modular benchmark CLI interface
 * Supports dynamic group selection and maintains compatibility with existing CI
 */

import { writeFileSync } from 'fs';
import { BenchmarkOptions, BenchmarkSection } from './types.js';
...
function parseArgs(): BenchmarkOptions & { group?: string; help?: boolean } {
  ...
  // Default to 'pr' if no group specified (fast CI mode)
  if (!group && !help) {
    group = 'pr';
  }
...
function showUsage(): void {
  ...
  console.log(`
Usage: node benchmark/new-index.js [options]
...
Group Options (select one):
  ...
  --fast        Fast benchmarks: quick cases with sparse solver only
...
Available Groups:`);
  ...
  console.log(`
Examples:
  node benchmark/new-index.js                     # PR benchmarks (default)
  node benchmark/new-index.js --release           # Release benchmarks
  node benchmark/new-index.js --full              # Full comparison
  node benchmark/new-index.js --fast              # Quick test
  node benchmark/new-index.js --pr --json=pr.json # PR with JSON output
  node benchmark/new-index.js --release --quiet   # Release mode, quiet output
`);
}
